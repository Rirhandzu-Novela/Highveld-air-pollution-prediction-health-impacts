...
"""
Single-Station CNN Production Model - Training and Evaluation
...
print("="*80)
print("SINGLE-STATION CNN MODEL - TRAINING WITH OPTIMAL PARAMETERS")
print("="*80)
...
try:
    with open('singlestation_cnn_best_params.json', 'r') as f:
...
# Save metrics
metrics_df.to_csv('Results/SingleStation_CNN_metrics_PM2.csv', index=False)
predictions_df.to_csv('Results/SingleStation_CNN_Predictions.csv', index=False)
print("\nMetrics saved to: Results/SingleStation_CNN_metrics_PM2.csv")
print("Predictions with intervals saved to: Results/SingleStation_CNN_Predictions.csv")
...
plt.tight_layout()
plt.savefig('Results/SingleStation_CNN_Analysis.png', dpi=300, bbox_inches='tight')
print("Analysis visualization saved to: Results/SingleStation_CNN_Analysis.png")
plt.close()

# Save model
model.save('singlestation_cnn_best_model.h5')
print("Model saved to: singlestation_cnn_best_model.h5")
...
if SHAP_AVAILABLE:
    ...
    try:
        ...
        def cnn_single_predict_wrapper(X_flat):
        ...
        explainer = shap.KernelExplainer(cnn_single_predict_wrapper, X_background)
        ...
        plt.title('Single-Station CNN Model SHAP Feature Importance (Top 20)', fontweight='bold', size=14)
        plt.tight_layout()
        plt.savefig('Results/singlestation_cnn_shap_summary.png', dpi=300, bbox_inches='tight')
        print("SHAP summary plot saved to: Results/singlestation_cnn_shap_summary.png")
        plt.close()
        ...
        plt.title('Single-Station CNN Model SHAP Feature Importance (Mean)', fontweight='bold', size=14)
        plt.tight_layout()
        plt.savefig('Results/singlestation_cnn_shap_bar.png', dpi=300, bbox_inches='tight')
        print("SHAP bar plot saved to: Results/singlestation_cnn_shap_bar.png")
        plt.close()
...
print("\n" + "="*80)
print("SINGLE-STATION CNN TRAINING COMPLETE")
print("="*80)
