"""
2D CNN-LSTM Production Model - Training and Evaluation
Uses best hyperparameters from grid search
Includes: Predictions, Metrics, Visualizations, Analysis
"""
...
print("="*80)
print("2D CNN-LSTM MODEL - TRAINING WITH OPTIMAL PARAMETERS")
print("="*80)
...
try:
    with open('2d_cnn_lstm_best_params.json', 'r') as f:
...
# ============================================================================
# 3. CREATE TEMPORAL 2D SEQUENCES
# ============================================================================
print("\n[3/7] Creating temporal 2D sequences...")

def create_temporal_2d_sequences(em_data, mb_data, target_data, n_in=1):
    X, y = [], []
    
    for i in range(len(em_data) - n_in):
        em_temporal = em_data[i:i+n_in]
        mb_temporal = mb_data[i:i+n_in]
        
        temporal_2d = []
        for t in range(n_in):
            stacked = np.vstack([em_temporal[t], mb_temporal[t]])
            stacked_2d = stacked.reshape(2, 12, 1)
            temporal_2d.append(stacked_2d)
        
        temporal_2d = np.array(temporal_2d)
        target_next = target_data[i+n_in]
        
        X.append(temporal_2d)
        y.append(target_next)
    
    return np.array(X), np.array(y)

X, y = create_temporal_2d_sequences(em_norm, mb_norm, target_norm,
...
model = keras.Sequential([
    layers.Input(shape=(X_train.shape[1], 2, 12, 1)),
    
    # TimeDistributed Conv2D for spatial feature extraction
    layers.TimeDistributed(
        layers.Conv2D(best_params['filters'], 3, padding='same', activation='relu')
    ),
    layers.TimeDistributed(layers.BatchNormalization()),
    layers.TimeDistributed(layers.Dropout(best_params['dropout'])),
...
# Save metrics
metrics_df.to_csv('Results/eMa2DCNNLSTM_metrics_PM2.csv', index=False)
print("\nMetrics saved to: Results/eMa2DCNNLSTM_metrics_PM2.csv")
...
plt.tight_layout()
plt.savefig('Results/2D_CNN_LSTM_Training_Analysis.png', dpi=300, bbox_inches='tight')
print("Training analysis saved to: Results/2D_CNN_LSTM_Training_Analysis.png")
...
predictions_df.to_csv('Results/2D_CNN_LSTM_Predictions.csv', index=False)
print("Predictions saved to: Results/2D_CNN_LSTM_Predictions.csv")

# Save model
model.save('2d_cnn_lstm_best_model.h5')
print("Model saved to: 2d_cnn_lstm_best_model.h5")

print("\n" + "="*80)
print("2D CNN-LSTM TRAINING COMPLETE")
print("="*80)
